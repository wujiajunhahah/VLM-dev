...
public class FastVLMProcessor: UserInputProcessor {
    ...
    public func prepare(prompt: UserInput.Prompt, imageTHW: THW?) -> String {
        // 将 Prompt 统一转换为模型需要的 [Message]
        let generator = DefaultMessageGenerator()
        var messages: [Message]
        switch prompt {
        case .text(let text):
            messages = generator.generate(messages: [.user(text)])
        case .messages(let raw):
            messages = raw
        case .chat(let chats):
            messages = generator.generate(messages: chats)
        }
        ...
        if let imageTHW {
            ...
            lastMessage += Array(repeating: config.imageToken, count: numImageTokens).joined()
        }
